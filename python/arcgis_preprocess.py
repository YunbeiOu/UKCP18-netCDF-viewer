...
# 1. Aggregate to seasonal climate projectins 
# set path
...
# define cell statistics function
...
# 4. Use Cell Statistics to create periodic mean for each variable
...
# 5.Change symbology
# 5.1 Input geotiff and save as .lyr file (layer file)
...
# 5.3 Use 'RasterStretchColorize' to set the stretch symbology for all layers (set min and max)
...
# 7. Change cache tiles (manage-map-server-cache-tiles)
...
# 8. Data querying - get values for each local authority 
# 8.1 Read the attribute names (NAME_2)
...
# 8.3 Band Collection Statistics
...
# 9.3 Add fields in attribute table
...
# 9.4 Loop the .txt to extract value and put in dataframe
...
# 9.5 Insert into attribute table in ArcGIS
